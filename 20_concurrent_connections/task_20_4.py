...
def send_cfg_command(device, command):
    with ConnectHandler(**device) as ssh:
        ssh.enable()
        prompt = ssh.find_prompt()
...
def send_commands_to_devices(devices, filename, limit=3, show=None, config=None):
    if show is None:   # if no show then config
        with ThreadPoolExecutor(limit) as executor:
            result = executor.map(send_cfg_command, devices, repeat(config))
            for info in result:
...
if __name__ == "__main__":
    ...
    send_commands_to_devices(devices, filename='result.txt', config=['router ospf 55', 'network 0.0.0.0 255.255.255.255 area 0'] )
