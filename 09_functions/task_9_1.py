    ...
    for int in intf_vlan_mapping:
        ...
        for command in access_template:
            if 'access v' in command:
                command_out.append(str(command) + ' ' + str(intf_vlan_mapping[int]))
            else:
                command_out.append(str(command))
    return(command_out)
    #print(command_out)  использовав для проверки для себя




#generate_access_config(access_config, access_mode_template) ето строка нужна для визова, но тест проходит и без нее
